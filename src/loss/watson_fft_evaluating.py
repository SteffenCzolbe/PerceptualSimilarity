...
class WatsonDistanceFftEvaluating(nn.Module):
    """
    Modified class of the watson-fft metric. Independent weighting of phase and amplitude 
    can be performed by changing the self.a and self.b parameters.
    """
    def __init__(self, blocksize=8, trainable=False, reduction='sum'):
        ...
        self.add_module('fft', Rfft2d(blocksize=blocksize.item(), interleaving=True))
        ...
        self.a = 1
        self.b = 1
        self.total_amplitude_term_dist = 0.
        self.total_phase_term_dist = 0.
    ...
    def forward(self, input, target):
        ...
        # get amplitudes
        c0_amp = torch.norm(c0, p='fro', dim=4)
        c1_amp = torch.norm(c1, p='fro', dim=4)
        ...
        # get phases
        c0_phase = torch.atan2( c0[:,:,:,:,1], c0[:,:,:,:,0])
        c1_phase = torch.atan2( c1[:,:,:,:,1], c1[:,:,:,:,0])
        
        # angular distance
        phase_dist = torch.acos(torch.cos(c0_phase - c1_phase)) * self.w_phase
        phase_dist = self.dropout(phase_dist)
        phase_dist = torch.sum(phase_dist, dim=(1,2,3))
        
        # perceptual distance
        distance = self.a * watson_dist + self.b * phase_dist
        self.total_amplitude_term_dist += torch.sum(self.a * watson_dist).item()
        self.total_phase_term_dist += torch.sum(self.b * phase_dist).item()
