...
parser.add_argument('--colorspace', type=str, default='RGB', help='[RGB], or [Gray] for color space')
parser.add_argument('--trainloss', type=str, default='ranked', help='[ranked], or [weightedsigmoid] architectures')
...
parser.add_argument('--args_blocksize', type=int, default=8, help='Arguments for Fourier based transforms')
parser.add_argument('--args_train_weights', type=str2bool, default=True, help='Arguments for Fourier based transforms')
parser.add_argument('--args_train_beta', type=str2bool, default=True, help='Arguments for Fourier based transforms')
...
# rebuild args
args = {}
args['blocksize'] = opt.args_blocksize
args['train_weights'] = opt.args_train_weights
args['train_beta'] = opt.args_train_beta

# initialize model
model = dm.DistModel()
model.initialize(model=opt.model,net=opt.net, colorspace=opt.colorspace, use_gpu=opt.use_gpu, is_train=True, lr=opt.lr ,train_loss=opt.trainloss, pnet_rand=opt.from_scratch, pnet_tune=opt.train_trunk, args=args)
