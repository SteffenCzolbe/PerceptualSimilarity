...
def tensor2tensorlab(image_tensor_batch,to_norm=True,mc_only=False):
    # image tensor to lab tensor
    from skimage import color
    N, _, _, _ = image_tensor_batch.shape

    for n in range(N):
        img = tensor2im(image_tensor_batch[0].unsqueeze(-1))
        ...
        if(to_norm and not mc_only):
            img_lab[:,:,0] = img_lab[:,:,0]-50
            img_lab = img_lab/100.
        image_tensor_batch[0,:,:,:] = np2tensor(img_lab)[0]
    return image_tensor_batch

def tensor2tensorGrayscale(image_tensor):
    # tensor of dim (N, 3, H, W) -> (N, 1, H, W)
    (N,C,X,Y) = image_tensor.size()
    return (image_tensor[:,0,:,:] * 0.3 + image_tensor[:,1,:,:] * 0.59 + image_tensor[:,2,:,:] * 0.11).view(N,1,X,Y)

def tensor2tensorGrayscaleLazy(image_tensor):
    # tensor of dim (N, 3, H, W) -> (N, 3, H, W)
    grayscale = tensor2tensorGrayscale(image_tensor)
    return torch.cat((grayscale, grayscale, grayscale), 1)
